...
PR: https://github.com/diyorbeknematov28042007-dotcom/Yuristim09092026/pull/11.
Merge/release status: see the accompanying delivery message.
...
PR review reproduced four additional failures: retry loss with paused/OPEN/admission-failing fallbacks (three cases), and final failure diagnostics incorrectly attributed to the initial provider. Deferred retries now preserve the allowance and recheck circuit admission; failure diagnostics carry the actual provider/model. Added protection against bypassing an opened circuit and exceeding retry count.
